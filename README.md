...
- 启动后先看顶部状态卡片，确认环境已就绪、可导出数量正常
- 在“导出格式”里选择普通音频文件、MP3 或 FLAC
- 保持“已选择 0 首”时点击“导出全部”，会按当前全局设置导出全部可导出歌曲
- 如果只想导出部分歌曲，可以在列表里勾选条目，或先用搜索框 / 质量筛选缩小范围

![浏览全部缓存界面](docs/screenshots/guide-overview-tight.png)

### 2. 搜索指定歌曲并等待缓存出现

- 切到“搜索指定歌曲”
- 输入歌名、`trackId`，必要时补艺人名
- 选择目标版本，例如 `lossless / flac`
- 先点“搜索匹配”确认命中结果；如果当前还没缓存到本地，再用“等待并导出”

![搜索指定歌曲界面](docs/screenshots/guide-search-mode.png)

### 3. 高级选项

- `Dry run` 只做解析和预检查，不真正写出文件
- `验证音频` 会在导出后做额外可播放性校验，适合排查转码或解密异常
- 这些选项在批量导出前打开即可，无需单独保存

![高级选项展开态](docs/screenshots/guide-advanced-options.png)
